...
def test_schema16_database_migrates_to_current_schema_without_losing_live_data(tmp_path: Path) -> None:
    ...
    migrated.connect()
    try:
        assert migrated.get_schema_version() == SCHEMA_VERSION == 25
    ...
    reopened.connect()
    try:
        assert reopened.get_schema_version() == 25
...
def test_schema16_migration_repairs_known_orphans_and_creates_backup(tmp_path: Path) -> None:
    ...
    try:
        ...
        backup_path = tmp_path / "metadata.db.pre-schema16-to-25-repair.bak"
...
def test_schema16_migration_preserves_unknown_foreign_key_violation(tmp_path: Path) -> None:
    ...
    assert (tmp_path / "metadata.db.pre-schema16-to-25-repair.bak").is_file()


def test_schema16_migration_matches_fresh_schema25_structure(tmp_path: Path) -> None:
