...
class ChatManager:
    ...
    def start(self):
        """启动后台任务"""
        if self._cleanup_task is not None or self._auto_save_task is not None:
            logger.warning("后台任务已经在运行，跳过重复启动")
            return
    ...
    async def _auto_save_task_loop(self):
        """定期自动保存所有聊天流"""
        try:
            while True:
...
def get_chat_manager(auto_start: bool = True):
    global chat_manager
