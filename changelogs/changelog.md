...
- 适配器访问策略新增相互独立的群聊与私聊默认动作；为兼容现有适配器，两者默认放行，也可改为默认拒绝，并继续支持聊天流级别的允许、拒绝和继承规则。
- 回复效果评估标准升级为 v3：无关后续消息使用空关联，修复候选回复跨消息合并造成的时序误归因；观察窗口延长至 30 分钟或 15 条后续消息，无针对 Bot 的情绪证据时接受度留空，低置信及多候选情绪向中性收缩，原始分按实际可用维度归一，并移除相对分。WebUI 仅按单一评估标准版本聚类和比较。
- Reply 工具将回复指引与关键信息合并为统一的 `reply_reference` 字段，减少重复信息和语义歧义。
- Reply 工具新增可选的 `reply_style` 参数，可为单次回复指定简短表达、正常回复或长回复。
- Replyer 将回复信息参考拆分为独立上下文 Item，避免与最终回复指令混在同一消息中。
- 修复回复效果评审错误继承 Planner 模型与工具上下文、网络超时被当作格式错误重复请求、串行结算阻塞和重启后 pending 记录不再恢复的问题；评审改用隔离的非推理 JSON 请求、60 秒总时限和有限并发队列。
- 修复回复效果页面按完整动态请求指纹聚合导致每个 Prompt 版本只有一条样本的问题，拆分完整请求指纹与稳定 Prompt 版本指纹并迁移已有记录；版本计算忽略聊天流专属额外要求及空格、换行等排版差异，仍支持按聊天流筛选、独立折叠模型/版本，以及查看真实 Prompt 和相对当前实发 Prompt 的差异。
- 回复效果页面新增“数据分析 / 评估浏览”双视图；评估浏览支持按聊天流、状态、策略、模型及置信度筛选，按时间和各项分数排序，并以左侧记录列表、右侧上下文与归因证据的布局逐条查看评估；未开启回复效果追踪调试开关时，侧边栏隐藏该页面入口。
- 回复效果完整记录与诊断镜像改为无损压缩存储，迁移时逐条校验内容并回收数据库空页，在保留全部上下文、Prompt 和生成诊断的同时显著降低空间占用。
- 回复效果页面支持将全部评分记录导出为可迁移的 JSON.GZ 文件并直接导入；重复记录自动跳过，ID 冲突不会覆盖本地数据。
- 回复效果页面新增带二次确认的“清空评分”，同步删除数据库记录与诊断镜像，并取消尚未结算的观察任务，避免旧评分被后台重新写回。
- 移除已停用的实验性网页浏览工具、配置和相关测试；插件的 HTML 转图片能力不受影响。
...
- 修复表达向量索引 JSON 损坏后历史补建反复异常重启的问题；损坏的生成索引会明确记录并从数据库重建，向量文件改为版本化 NPZ 并由 JSON 原子切换，失败任务增加重试冷却。
- 优化表达向量索引在线维护：新增、历史回填、内容变化和失败恢复统一按最近聚类中心增量分配；索引首次追平时从零正式聚类一次，进入稳定状态后累计变化达到 5% 才使用旧中心全量重聚类；同时优化 k-means++、后台计算和无压缩原子写盘，避免小批量学习反复重算全库。
- 表达向量回填支持单条嵌入失败隔离：成功项照常入库，失败项冷却后重试并在反复失败时低频隔离，不阻塞其他表达和首次稳定化；恢复项进入普通 5% 变化统计，Provider 整体故障不会被误判为坏数据，索引异常也不再影响表达学习和普通候选回退。

## Webui [1.7.0]

- 修复移动端保存模型额外参数时，触摸事件连带关闭模型编辑弹窗的问题。
- 修复移动端模型设置子标签内容被裁切且无法上下滚动的问题。
- 修复长期记忆人物画像列表高度固定，在高分辨率屏幕下无法充分利用页面空间的问题。
...
- 插件 `send.text`、`send.emoji`、`send.image`、`send.forward`、`send.hybrid`、`send.command` 和 `send.custom` 支持通过 `return_details=True` 获取平台确认的最终消息 ID，默认返回值仍为布尔值。
