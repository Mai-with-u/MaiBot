...
- 修复全局 AI 搜索虚构配置文件、字段和数值的问题；技术项必须在本次实际检索或读取的证据中出现，校验失败时会先自动移除无依据内容并重写一次。
- 全局 AI 搜索支持实时逐条展示分析轮次、检索词、文档读取目标、命中结果及最终成功或失败状态；回答完成后自动折叠过程，并可手动再次展开。
- 全局 AI 搜索会在主日志中临时保留查询、检索步骤、来源与结果状态，并沿用日志大小轮转和过期清理策略自动删除旧记录。
- 优化插件版本不兼容提示，有兼容更新时可直接从插件管理中更新。
