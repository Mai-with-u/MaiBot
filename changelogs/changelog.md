...
- 新增可选的交互式终端输入，可在 Bot 运行期间发送普通消息及 `/clear`、`/pm` 等指令，支持 Tab 补全、输入历史和日志安全重绘。
- 新增 `/offline` 和 `/online` 指令，可临时关闭并恢复全部适配器插件，并复用插件管理权限控制。
- 本地控制台输出使用精确本地路由，不再追加 legacy 发送或打印外部平台投递日志。
- 控制台 `/clear` 改为通过 Tab 选择真实聊天名，不再清理控制台自身的临时会话。
- 控制台新增 `/help` 指令，集中展示管理指令与输入快捷键。

## A_Memorix

- 修复旧版记忆数据库存在孤立关联时 Schema 自动迁移失败的问题，迁移会先备份数据库并定向清理无法使用的关联数据。
...
- 修复表达簇计算出错
- 修复 Embedding 浮点波动导致表达向量索引反复全量回填的问题，并通过持久化探针相似度连续确认真实向量空间漂移。
