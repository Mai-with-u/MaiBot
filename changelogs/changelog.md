...
- 优化部分页面配色和布局
- 新增单独的存储管理页面
- 首页活动卡片支持翻转查看最近 24 小时在线时长图表
- 首页一言支持停用默认内容并维护自定义内容列表
- 修复全局顶栏默认背景色比页面背景偏暗的问题
- 麦麦观察中的引用回复支持跳转并高亮原始消息
- 修复麦麦观察首次补发大量积压事件时会话列表跳动、时间线反复滚动的问题
- 精简麦麦聊天会话侧栏，移除重复的标题和会话数量
- 扩展未来复古外观设置，新增纸张暖度、五种纸面纹理、纹理强度、面板层次和描边力度调节
- 修复悬浮侧栏展开与收起时选中块及导航图标发生水平、垂直位移的问题
- 修复首页活动卡片悬停反馈不明显、翻转上沿被裁切及投影样式不统一的问题
